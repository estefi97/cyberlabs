...
<title>Editar Módulo | Hacktiva</title>
            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">Editar Módulo</h2>
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="/dashboard">{{ __("DashboardComentarios3") }}</a>
                                    </li>
                                    <li class="breadcrumb-item"><a href="/dashboard/modulos-cursos">{{ __("DashboardComentarios1") }}</a>
                                    </li>
                            ...
                            <div class="card-body">
                                <div class="tab-content">
                                    <div class="tab-pane active" id="account" aria-labelledby="account-tab" role="tabpanel">
                                        ...
                                        <form action="{{ route('dashboard_moduloscursos.editar') }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            <div class="row">
                                                <div class="col-12 col-sm-12">
                                                    <div class="form-group">
                                                        <div class="controls">
                                                            <label class="label-dashboard">Nombre del Módulo</label>
                                                            <input name="nombreModuloCurso" id="nombreModuloCurso" value="{{ $module->name }}" placeholder="Ingrese el Nombre del Módulo" type="text" class="form-control" autocomplete="off" >
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="controls">
                                                            <label class="label-dashboard">Estado</label>
                                                            <select name="estadoModuloCurso" class="form-control" id="basicSelect">
                                                                <option value="1" @if($module->status === "1") {{ __("selected") }} @endif>PUBLICADO</option>
                                                                <option value="2" @if($module->status === "2") {{ __("selected") }} @endif>ELIMINADO</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="controls">
                                                            ...
                                                            <select name="cursosAsociadosModuloCurso[]" class="select2 form-control" multiple="multiple">
                                                                @foreach(DB::table('courses')->select('id','name')->get() as $course)
                                                                    <option value="{{ $course->id }}" @if(DB::table('module_course')->where([['module_id','=',$module->id],['course_id','=',$course->id]])->select('course_id')->get()->count() > 0) {{ __("selected") }} @endif>{{ $course->name }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="controls">
                                                            <label class="label-dashboard">Descripción del Desafío</label>
                                                            <textarea class="ckeditor" name="descripcionDesafioModuloCurso" id="editor1" rows="10" cols="80">{{ $module->description_challenge }}</textarea>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="controls">
                                                            <label class="label-dashboard">Solución del Desafío</label>
                                                            <input name="solucionDesafioModuloCurso" id="solucionDesafioModuloCurso" value="{{ $module->solution_challenge }}" placeholder="Ingrese la Solución del Desafío" type="text" class="form-control" autocomplete="off" >
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="controls">
                                                            <label class="label-dashboard">Contenido del Módulo</label>
                                                            <textarea class="ckeditor" name="contenidoModuloCurso" id="editor1" rows="10" cols="80">{{ $module->content }}</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-12 d-flex flex-sm-row flex-column justify-content-end mt-1">
                                                    <input type="hidden" name="idModulo" value="{{ $module->id }}">
