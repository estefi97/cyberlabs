...
<title>{{ __("DashboardArticulos1") }} | Hacktiva</title>
@section('content')
            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">{{ __("DashboardArticulos2") }}</h2>
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="/dashboard">{{ __("DashboardArticulos3") }}</a>
                                    </li>
                                    <li class="breadcrumb-item active">{{ __("DashboardArticulos2") }}
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <section id="data-list-view" class="data-list-view-header">
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            <thead>
                            <tr>
                                <th style="text-align: center;">{{ __("DashboardArticulos4") }}</th>
                                <th style="text-align: center;">{{ __("DashboardArticulos5") }}</th>
                                <th style="text-align: center;">{{ __("DashboardArticulos6") }}</th>
                                <th style="text-align: center;">{{ __("DashboardArticulos7") }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @forelse($article->withTrashed()->orderBy('id','DESC')->get() as $article)
                                <tr>
                                    ...
                                    <td style="text-align: center;">{{ $article->created_at->format('d-m-Y') }}</td>
                                    <td style="text-align: center;">
                                        <a href="{{ route('dashboard_articulos_editar', $article->id) }}"><span class="editBtn"><i class="feather icon-edit"></i></span></a>
                                        @if ($article->status === "1")
                                            <span class="deleteBtn"><i class="feather icon-trash"></i></span>
                                            <span style="display: none;" id="idArticuloSeleccionado">{{ $article->id }}</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
