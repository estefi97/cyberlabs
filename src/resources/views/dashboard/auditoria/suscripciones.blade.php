...
<title>Auditoría Suscripciones | Hacktiva</title>
@section('content')
            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">Auditoría Suscripciones</h2>
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="/dashboard">{{ __("DashboardArticulos3") }}</a>
                                    </li>
                                    <li class="breadcrumb-item active">Auditoría Suscripciones
                                    </li>
                                </ol>
                            </div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            ...
                            <tr>
                                ...
                                <th style="text-align: center;">Fecha Eliminada</th>
                            </tr>
                            </thead>
                            <tbody>
                            @forelse(DB::table('subscriptions')->select('stripe_plan','user_id','name','created_at','updated_at','ends_at')->orderBy('updated_at','DESC')->get() as $subscription)
                                <tr>
                                    <td style="text-align: center;">{{ $subscription->stripe_plan }}</td>
                                    <td style="text-align: center;">{{ DB::table('users')->select('name')->where('id','=',$subscription->user_id)->first()->name }} {{ DB::table('users')->select('last_name')->where('id','=',$subscription->user_id)->first()->last_name }}</td>
                                    <td style="text-align: center;">{{ $subscription->name }}</td>
                                    <td style="text-align: center;">{{ $subscription->created_at }}</td>
                                    <td style="text-align: center;">{{ $subscription->updated_at }}</td>
                                    <td style="text-align: center;">{{ $subscription->ends_at }}</td>
                                </tr>
                            @empty
    ...
    <!-- BEGIN: Page JS-->
    <script src="{{ asset('/dash/app-assets/js/scripts/ui/auditoria/suscripciones-data-list-view.js') }}"></script>
    ...
    <script>
        ...
        let menuAuditoriaSuscripciones = document.getElementById('menuAuditoriaSuscripciones');
        menuInicio.classList.remove('active');
        menuAuditoriaSuscripciones.classList.add('active');
    </script>
