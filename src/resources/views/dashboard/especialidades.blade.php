...
<title>{{ __("DashboardEspecialidades1") }} | Hacktiva</title>
@section('content')
            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">{{ __("DashboardEspecialidades2") }}</h2>
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="/dashboard">{{ __("DashboardEspecialidades3") }}</a>
                                    </li>
                                    <li class="breadcrumb-item active">{{ __("DashboardEspecialidades2") }}
                                    </li>
                                </ol>
                            </div>
                        </div>
                ...
                <section id="data-list-view" class="data-list-view-header">
                    ...
                    <div class="table-responsive">
                        <table class="table data-list-view">
                            <thead>
                            <tr>
                                <th style="text-align: center;">{{ __("DashboardEspecialidades4") }}</th>
                                <th style="text-align: center;">{{ __("DashboardEspecialidades14") }}</th>
                                <th style="text-align: center;">{{ __("DashboardEspecialidades5") }}</th>
                                <th style="text-align: center;">{{ __("DashboardEspecialidades6") }}</th>
                                <th style="text-align: center;">{{ __("DashboardEspecialidades7") }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @forelse($specialty->withTrashed()->orderBy('id','DESC')->get() as $specialty)
                                <tr>
                                    <td style="text-align: center;">{{ $specialty->name }}</td>
                                    <td style="text-align: center;">{{ $specialty->level->name }}</td>
                                    <td style="text-align: center;">
                                        @if ($specialty->status === "1")
                                            {{ __("DashboardEspecialidades10") }}
                                        @endif
                                        @if ($specialty->status === "3")
                                            {{ __("DashboardEspecialidades11") }}
                                        @endif
                                    </td>
                                    <td style="text-align: center;">{{ $specialty->created_at->format('d-m-Y') }}</td>
                                    <td style="text-align: center;">
                                        <a href="{{ route('dashboard_especialidades_editar', $specialty->id) }}"><span class="editBtn"><i class="feather icon-edit"></i></span></a>
                                        @if ($specialty->status === "1")
                                            <span class="deleteBtn"><i class="feather icon-trash"></i></span>
                                            <span style="display: none;" id="idEspecialidadSeleccionada">{{ $specialty->id }}</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __("DashboardEspecialidades8") }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('dashboard_especialidades.eliminar') }}" method="POST">
                    @csrf
                        <div class="modal-body">
                            <input type="hidden" name="deleteId" id="deleteId">
                            <h4>{{ __("DashboardEspecialidades9") }}</h4>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __("DashboardEspecialidades13") }}</button>
                            <button type="submit" class="btn btn-primary" name="deleteData">{{ __("DashboardEspecialidades12") }}</button>
                        </div>
                </form>
            </div>
        </div>
    ...
    <!-- BEGIN: Page JS-->
    <script src="{{ asset('/dash/app-assets/js/scripts/ui/especialidades/data-list-view.js') }}"></script>
